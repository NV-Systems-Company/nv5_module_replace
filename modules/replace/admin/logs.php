...
$xtpl->assign('OP', $op);
$xtpl->assign('checksess', md5('replace_' . NV_CHECK_SESSION . '_' . $admin_info['userid']));
...
foreach ($data as $data_i) {
    ...
    if ($logs_del) {
        $xtpl->assign('DEL_URL', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=logs_del&amp;id=' . $data_i['id']);
